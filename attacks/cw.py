...
def cw(x, model_fn, y=None, eps=1.0, ord_=2, T=2,
    ...
    :param min_prob: The minimum confidence of adversarial examples.
        Generally larger min_prob wil result in more noise.
    ...
    clip_min = float(config['cw_clip_min'])
    clip_max = float(config['cw_clip_max'])
    ...
    # scale input to (0, 1)
    x_scaled = (x - clip_min) / (clip_max - clip_min)
    ...
    xadv = xadv * (clip_max - clip_min) + clip_min
    ...
    if 2 == ord_:
        ...
        # CW-Linf
        tau0 = tf.fill([xshape[0]] + [1] * len(axis), clip_max)
