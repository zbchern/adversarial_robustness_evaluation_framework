...
def jsma(x, model_fn, n_classes, score_fn=lambda t, o: t * tf.abs(o)):
    """
    ...
    epsilon = float(config['jsma_epsilon'])
    epochs = config['jsma_epochs']
    k = config['jsma_k']
    clip_min = float(config['jsma_clip_min'])
    clip_max = float(config['jsma_clip_max'])

    n = tf.shape(x)[0]
    y = tf.random_uniform(shape=[1], minval=0, maxval=n_classes + 1, dtype=tf.int32)
    target = tf.zeros([n], dtype=tf.int32) + y[0]
    ...
    return _jsma_fn(x, model_fn, target, epochs=epochs, eps=epsilon,
...
def _jsma_impl(x, model_fn, yind, epochs, eps, clip_min, clip_max, score_fn):
    loop_vars = [0, tf.identity(x)]

    def loop_cond(idx, _):
        return idx < epochs

    def loop_body(idx, xadv):
        logits, _ = model_fn(xadv)
    ...
    _, xadv = tf.while_loop(loop_cond, loop_body, loop_vars,
                            back_prop=False, name='jsma_x1_batch')

    return xadv


def _jsma2_impl(x, model_fn, yind, epochs, eps, clip_min, clip_max, score_fn):

    loop_vars = [0, tf.identity(x)]

    def loop_cond(idx, _):
        return tf.less(idx, epochs)
    ...
    _, xadv = tf.while_loop(loop_cond,
                            loop_body,
                            loop_vars,
                            back_prop=False,
                            name='jsma_x2_batch')
